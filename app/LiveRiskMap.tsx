...
function markerLabel(severity: Severity) { if (severity === "CRITICAL") return "CRIT"; if (severity === "ALERT") return "ALRT"; if (severity === "WATCH") return "WATCH"; return "INFO"; }
function riskScore(event: EventItem) { const base = event.severity === "CRITICAL" ? 88 : event.severity === "ALERT" ? 72 : event.severity === "WATCH" ? 54 : 28; const boost = event.category === "Conflict" ? 10 : event.category === "Energy" ? 8 : event.category === "Government" ? 6 : 0; return Math.round(clamp(base + boost, 0, 99)); }
function buildClusters(events: EventItem[]) { const groups = new Map<string, { label: string; count: number; score: number; lat: number; lon: number }>(); for (const event of events) { const label = event.category || "Unclassified"; const row = groups.get(label) || { label, count: 0, score: 0, lat: 0, lon: 0 }; row.count += 1; row.score += riskScore(event); row.lat += event.lat; row.lon += event.lon; groups.set(label, row); } return Array.from(groups.values()).map((row) => ({ ...row, score: Math.round(row.score / Math.max(1, row.count)), lat: row.lat / Math.max(1, row.count), lon: row.lon / Math.max(1, row.count) })).sort((a, b) => b.score * b.count - a.score * a.count); }
...
export function LiveRiskMap({ events }: { events: EventItem[] }) {
  ...
  const filteredEvents = useMemo(() => events.filter((event) => (severityFilter === "All" || event.severity === severityFilter) && (categoryFilter === "All" || event.category === categoryFilter) && (sourceFilter === "All" || event.source === sourceFilter)), [categoryFilter, events, severityFilter, sourceFilter]);
  const hotspots = useMemo(() => [...filteredEvents].sort((a, b) => riskScore(b) - riskScore(a)).slice(0, 12), [filteredEvents]);
  ...
  const visibleEvents = useMemo(() => filteredEvents.slice(0, 180).map((event, index) => { const point = project(event.lat, event.lon, zoom); return { event, index, left: point.x - viewport.startX, top: point.y - viewport.startY }; }), [filteredEvents, zoom, viewport.startX, viewport.startY]);
  ...
  return (
    <div className="mapCanvas realMapCanvas">
      ...
      <div className="mapFilterBar" onPointerDown={(event) => event.stopPropagation()} onWheel={(event) => event.stopPropagation()}>
        <select value={severityFilter} onChange={(event) => setSeverityFilter(event.target.value as (typeof severityOptions)[number])}>{severityOptions.map((item) => <option key={item}>{item}</option>)}</select>
        <select value={categoryFilter} onChange={(event) => setCategoryFilter(event.target.value)}>{categories.map((item) => <option key={item}>{item}</option>)}</select>
        <select value={sourceFilter} onChange={(event) => setSourceFilter(event.target.value)}>{sources.map((item) => <option key={item}>{item}</option>)}</select>
      </div>
      <div ref={viewportRef} className="realMapViewport" role="application" aria-label="Interactive OpenStreetMap global risk map" onPointerDown={onPointerDown} onPointerMove={onPointerMove} onPointerUp={onPointerUp} onPointerCancel={onPointerUp} onWheel={onWheel}>
        ...
        {visibleEvents.map(({ event, index, left, top }) => { if (left < -40 || top < -40 || left > size.width + 40 || top > size.height + 40) return null; const hot = riskScore(event) >= 72; return <button key={`${event.source}-${index}-${event.title}`} className={`mapMarker realMarker ${sevClass(event.severity)} ${hot ? "riskHot" : ""} ${selectedIndex === index ? "selected" : ""}`} style={{ left, top }} title={`${event.title}\n${event.source}\n${event.impact}`} onClick={(click) => { click.stopPropagation(); setSelectedIndex(index); }}><span className="markerPulse" /><span className="markerCore" /><span className="markerLabel">{markerLabel(event.severity)}</span></button>; })}
      ...
      <div className="mapHotspotStack" onPointerDown={(event) => event.stopPropagation()}><header><b>Hotspots</b><span>{hotspots.length} ranked</span></header>{hotspots.slice(0, 6).map((event) => <button key={`${event.source}-${event.title}`} onClick={() => focusEvent(event)}><span className={sevClass(event.severity)}>{event.severity}</span><b>{event.title}</b><em>{riskScore(event)}</em></button>)}</div>
      <div className="mapClusterStack" onPointerDown={(event) => event.stopPropagation()}>{clusters.slice(0, 5).map((cluster) => <button key={cluster.label} onClick={() => { setCenter({ lat: cluster.lat, lon: cluster.lon }); setZoom(4); }}><span>{cluster.label}</span><b>{cluster.score}</b><small>{cluster.count} events</small></button>)}</div>
      ...
      {selected ? <a className={`mapPopup liveEventPopup ${sevClass(selected.severity)}`} href={selected.url} target="_blank" rel="noreferrer" style={selectedPosition ? { "--pin-x": `${selectedPosition.left}px`, "--pin-y": `${selectedPosition.top}px` } as React.CSSProperties : undefined}><strong>{selected.severity} | {selected.category} | Risk {riskScore(selected)}/100</strong><b>{selected.title}</b><small>{selected.source} | {formatTime(selected.publishedAt)}</small><p>{selected.impact || selected.summary}</p><em>Open source report</em></a> : null}

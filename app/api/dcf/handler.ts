...
type AlphaStatement = { annualReports?: Array<Record<string, unknown>> };
type SecFacts = { facts?: Record<string, Record<string, { units?: Record<string, Array<Record<string, unknown>>> }>> };
type FinnhubProfile = { name?: string; finnhubIndustry?: string; marketCapitalization?: number; shareOutstanding?: number; weburl?: string };
...
export async function GET(request: Request) {
  ...
  if (!symbol) return NextResponse.json(unavailable(symbol, sources, "Search a valid public ticker to build the DCF."));

  const finnhubKey = process.env.FINNHUB_API_KEY;
  const alphaKey = process.env.ALPHA_VANTAGE_API_KEY;
  const fredKey = process.env.FRED_API_KEY;

  const [quote, finnhubProfile, overview, income, cashFlow, balance, riskFree] = await Promise.all([
    configured(finnhubKey) ? finnhubQuote(symbol, finnhubKey!, sources) : Promise.resolve(null),
    configured(finnhubKey) ? finnhubCompanyProfile(symbol, finnhubKey!, sources) : Promise.resolve(null),
    configured(alphaKey) ? alphaOverview(symbol, alphaKey!, sources) : Promise.resolve(null),
  ...
  const [news, filings, companyFacts] = await Promise.all([
    tickerNews(symbol, text(overview?.Name) || text(finnhubProfile?.name) || symbol, sources),
  ...
  const companyName = text(overview?.Name) || text(finnhubProfile?.name) || symbol;
  const profileShareCount = number(finnhubProfile?.shareOutstanding) == null ? null : number(finnhubProfile?.shareOutstanding)! * 1_000_000;
  const overviewShares = number(overview?.SharesOutstanding);
  const marketCap = number(overview?.MarketCapitalization) ?? (number(finnhubProfile?.marketCapitalization) == null ? null : number(finnhubProfile?.marketCapitalization)! * 1_000_000) ?? (price != null && (overviewShares ?? profileShareCount) ? price * (overviewShares ?? profileShareCount)! : null);
  const shares = overviewShares ?? profileShareCount ?? latest?.shares ?? (marketCap != null && price ? marketCap / price : null);
  ...
  const scenarios = buildScenarios({ revenue, revenueGrowthTrend, fcfMargin, netDebt: modeledNetDebt, shares, price, wacc, latest });
  ...
  const research = buildResearch({ symbol, companyName, overview, historical, scenarios, news, filings, wacc, sources });
  ...
  return NextResponse.json({
    ...
    profile: {
      ...
      industry: text(overview?.Industry) || text(finnhubProfile?.finnhubIndustry) || null,
    ...
    sourceLinks: [
      ...
      finnhubProfile?.weburl ? { label: "Company website", url: finnhubProfile.weburl } : null,
...
async function finnhubCompanyProfile(symbol: string, token: string, sources: Record<string, SourceState>) {
  try {
    const profile = await fetchJson<FinnhubProfile>(`https://finnhub.io/api/v1/stock/profile2?symbol=${encodeURIComponent(symbol)}&token=${encodeURIComponent(token)}`);
    sources.FinnhubProfile = profile?.name || profile?.marketCapitalization || profile?.shareOutstanding ? "ok" : "unavailable";
    return profile;
  } catch {
...
async function alphaOverview(symbol: string, apiKey: string, sources: Record<string, SourceState>) {
  try {
    const payload = await fetchJson<Record<string, string>>(`https://www.alphavantage.co/query?function=OVERVIEW&symbol=${encodeURIComponent(symbol)}&apikey=${encodeURIComponent(apiKey)}`);
    const usable = Boolean(payload?.Name || payload?.MarketCapitalization || payload?.RevenueTTM);
    sources.AlphaVantageOverview = usable ? "ok" : "unavailable";
...
async function secTickerCik(symbol: string, sources: Record<string, SourceState>) {
  try {
    const payload = await fetchJson<Record<string, { cik_str?: number; ticker?: string }>>("https://www.sec.gov/files/company_tickers.json", secHeaders());
...
async function tickerNews(symbol: string, companyName: string, sources: Record<string, SourceState>) {
  const out: NewsRow[] = [];
  const finnhubKey = process.env.FINNHUB_API_KEY;
  ...
  if (configured(finnhubKey)) {
    try {
      ...
      for (const row of (Array.isArray(rows) ? rows : []).slice(0, 28)) out.push(newsRow({ title: text(row.headline), source: text(row.source) || "Finnhub", url: text(row.url), summary: text(row.summary), publishedAt: typeof row.datetime === "number" ? new Date(row.datetime * 1000).toISOString() : new Date().toISOString() }));
      sources.FinnhubTickerNews = out.some((row) => row.source === "Finnhub") ? "ok" : "unavailable";
    } catch {
      sources.FinnhubTickerNews = "error";
    }
  } else sources.FinnhubTickerNews = "missing_key";
  ...
  if (configured(newsApiKey)) {
    try {
      ...
      for (const item of payload?.articles ?? []) out.push(newsRow({ title: text(item.title), source: text((item.source as Record<string, unknown> | undefined)?.name) || "NewsAPI", url: text(item.url), summary: text(item.description), publishedAt: text(item.publishedAt) || new Date().toISOString() }));
      sources.NewsAPITicker = (payload?.articles ?? []).length ? "ok" : "unavailable";
    } catch {
      sources.NewsAPITicker = "error";
    }
  } else sources.NewsAPITicker = "missing_key";
  ...
  if (configured(worldNewsKey)) {
    try {
      ...
      for (const item of payload?.news ?? []) out.push(newsRow({ title: text(item.title), source: text(item.source) || text(item.news_site) || "World News API", url: text(item.url), summary: text(item.summary) || text(item.text), publishedAt: text(item.publish_date) || text(item.published_at) || new Date().toISOString() }));
      sources.WorldNewsTicker = (payload?.news ?? []).length ? "ok" : "unavailable";
    } catch {
      sources.WorldNewsTicker = "error";
    }
  } else sources.WorldNewsTicker = "missing_key";
...
async function secFilings(symbol: string, cik: string, sources: Record<string, SourceState>) {
  ...
  try {
    ...
    for (let index = 0; index < Math.min(12, recent?.accessionNumber?.length || 0); index += 1) {
      ...
      rows.push({ title: `${symbol} ${form} filing`, source: "SEC EDGAR", url: `https://www.sec.gov/Archives/edgar/data/${Number(cik)}/${accession.replace(/-/g, "")}/${primary}`, category: "SEC Filing", severity: ["8-K", "10-K", "10-Q", "S-1", "S-3"].includes(form) ? "WATCH" : "INFO", publishedAt: text(recent?.filingDate?.[index]) || new Date().toISOString(), summary: text(recent?.primaryDocDescription?.[index]) || primary || "SEC filing" });
    }
...
function buildAlphaHistorical(income: AlphaStatement | null, cashFlow: AlphaStatement | null, balance: AlphaStatement | null) {
  ...
  return ascending.map((incomeRow, index) => {
    ...
    return makeYear({ date, revenue, previousRevenue, grossProfit: field(incomeRow, ["grossProfit"]), ebitda: field(incomeRow, ["ebitda"]) ?? (ebit != null && depreciation != null ? ebit + depreciation : null), ebit, netIncome: field(incomeRow, ["netIncome"]), pretax: field(incomeRow, ["incomeBeforeTax"]), tax: field(incomeRow, ["incomeTaxExpense"]), interestExpense: field(incomeRow, ["interestExpense"]), ocf: field(cashRow, ["operatingCashflow", "operatingCashFlow"]), capex: absOrNull(field(cashRow, ["capitalExpenditures"])), cash: field(balanceRow, ["cashAndCashEquivalentsAtCarryingValue", "cashAndShortTermInvestments"]), totalDebt, currentAssets: field(balanceRow, ["totalCurrentAssets"]), currentLiabilities: field(balanceRow, ["totalCurrentLiabilities"]), shares: field(balanceRow, ["commonStockSharesOutstanding"]) });
  }).filter((row) => row.revenue != null || row.freeCashFlow != null).sort(sortDescDate).slice(0, 8);
}
...
function buildSecHistorical(payload: SecFacts | null) {
  ...
  const shares = secFactMap(payload, ["EntityCommonStockSharesOutstanding"], "shares");
  ...
  return years.map((year, index) => {
    ...
    return makeYear({ date: `${year}-12-31`, revenue: revenue.get(year) ?? null, previousRevenue: index > 0 ? revenue.get(years[index - 1]) ?? null : null, grossProfit: grossProfit.get(year) ?? null, ebitda: null, ebit: ebit.get(year) ?? null, netIncome: netIncome.get(year) ?? null, pretax: pretax.get(year) ?? null, tax: tax.get(year) ?? null, interestExpense: interest.get(year) ?? null, ocf: ocf.get(year) ?? null, capex: absOrNull(capex.get(year) ?? null), cash: cash.get(year) ?? null, totalDebt: debt, currentAssets: currentAssets.get(year) ?? null, currentLiabilities: currentLiabilities.get(year) ?? null, shares: shares.get(year) ?? null });
  }).filter((row) => row.revenue != null || row.freeCashFlow != null).sort(sortDescDate).slice(0, 8);
}
...
function secFactMap(payload: SecFacts, concepts: string[], unit = "USD") {
  ...
  for (const concept of concepts) {
    ...
    for (const row of rows) {
      ...
      const annual = unit === "shares" || text(row.fp) === "FY" || form.startsWith("10-K") || /^CY\d{4}$/.test(frame);
...
function makeYear(input: { date: string; revenue: number | null; previousRevenue: number | null; grossProfit: number | null; ebitda: number | null; ebit: number | null; netIncome: number | null; pretax: number | null; tax: number | null; interestExpense: number | null; ocf: number | null; capex: number | null; cash: number | null; totalDebt: number | null; currentAssets: number | null; currentLiabilities: number | null; shares: number | null }) {
  ...
  return { fiscalYear: input.date ? input.date.slice(0, 4) : "Unavailable", fiscalDate: input.date || "", revenue: input.revenue, revenueGrowth: input.revenue != null && input.previousRevenue ? input.revenue / input.previousRevenue - 1 : null, grossProfit: input.grossProfit, grossMargin: ratio(input.grossProfit, input.revenue), ebitda: input.ebitda, ebit: input.ebit, ebitMargin: ratio(input.ebit, input.revenue), netIncome: input.netIncome, incomeBeforeTax: input.pretax, taxes: input.tax, taxRate: input.pretax && input.tax != null ? clamp(input.tax / input.pretax, 0, 0.5) : null, interestExpense: input.interestExpense == null ? null : Math.abs(input.interestExpense), operatingCashFlow: input.ocf, capex: input.capex, freeCashFlow: fcf, fcfMargin: ratio(fcf, input.revenue), cash: input.cash, totalDebt: input.totalDebt, netDebt: input.totalDebt != null && input.cash != null ? input.totalDebt - input.cash : null, workingCapital: input.currentAssets != null && input.currentLiabilities != null ? input.currentAssets - input.currentLiabilities : null, shares: input.shares } satisfies FinancialYear;
}
...
function buildScenarios(input: { revenue: number | null; revenueGrowthTrend: number | null; fcfMargin: number | null; netDebt: number | null; shares: number | null; price: number | null; wacc: WaccDetail; latest: FinancialYear | null }) {
...
function buildSensitivity(input: { revenue: number | null; revenueGrowth: number | null; fcfMargin: number | null; netDebt: number | null; shares: number | null; wacc: number | null }) {
  ...
  return { waccValues, terminalGrowthValues, rows: waccValues.map((wacc) => ({ wacc, values: terminalGrowthValues.map((terminalGrowthRate) => ({ terminalGrowthRate, valuePerShare: calculateDcf({ revenue: input.revenue, revenueGrowth: growth, fcfMargin: input.fcfMargin, wacc, terminalGrowth: Math.min(terminalGrowthRate, wacc - 0.005), netDebt: input.netDebt, shares: input.shares })?.valuePerShare ?? null })) })) };
}
...
function buildResearch(input: { symbol: string; companyName: string; overview: Record<string, string> | null; historical: FinancialYear[]; scenarios: Scenario[]; news: NewsRow[]; filings: NewsRow[]; wacc: WaccDetail; sources: Record<string, SourceState> }) {
  ...
  const catalysts = [base?.upsideDownsidePercent != null && base.upsideDownsidePercent > 0 ? `Base-case upside of ${roundPercent(base.upsideDownsidePercent)} if source-derived assumptions hold.` : null, input.overview?.AnalystTargetPrice ? `Analyst target price from Alpha Vantage: ${input.overview.AnalystTargetPrice}.` : null, latest?.freeCashFlow != null && latest.freeCashFlow > 0 ? "Latest reported free cash flow is positive." : null, input.news.length ? `${input.news.length} recent ticker-related news rows available for review.` : null].filter((item): item is string => Boolean(item));
  ...
  return { valuationSummary: base?.intrinsicValuePerShare != null ? `${input.symbol} base-case intrinsic value is ${roundMoney(base.intrinsicValuePerShare)} versus the current quote, with ${roundPercent(base.upsideDownsidePercent)} implied upside/downside.` : "DCF valuation is unavailable until revenue, FCF margin, WACC, net debt, shares, and price are available from providers.", investmentMemo: `${input.companyName || input.symbol} model uses provider-backed statement history, quote data, FRED rate context, SEC filings, and ticker news where available. Treat the output as probabilistic intelligence, not financial advice. The most important manual checks are FCF quality, debt/dilution, terminal value sensitivity, and recent filings/news.`, businessSummary: text(input.overview?.Description) || "Business description unavailable from Alpha Vantage for this ticker.", newsSummary: input.news.length ? `${input.news.length} ticker news rows loaded; ${severeNews.length} are above INFO severity.` : "No ticker-specific news rows returned by configured providers.", filingSummary: input.filings.length ? `${input.filings.length} recent SEC filings loaded. Review WATCH filings first.` : "No SEC filings returned. This can happen if CIK is unavailable or SEC access is not configured.", macroSummary: input.wacc.wacc != null ? `WACC is ${roundPercent((input.wacc.wacc ?? 0) * 100)}, using risk-free rate ${roundPercent((input.wacc.riskFreeRate ?? 0) * 100)}, beta ${input.wacc.beta?.toFixed(2) ?? "unavailable"}, ERP 5.5%, and source-derived capital structure where available.` : "WACC unavailable because market cap, beta, debt, or rate inputs were missing.", catalysts: catalysts.length ? catalysts : ["No automatic catalysts detected from available provider-backed rows."], risks, whatToVerify: ["Revenue, FCF, shares, and net debt against the latest 10-K/10-Q.", "Stock-based compensation, one-time items, leases, and adjusted EBITDA quality.", "Segment growth, customer concentration, geographic exposure, and supply-chain risk.", "Peer multiples and whether the terminal value is too large a share of the DCF.", "Recent SEC filings, legal/regulatory headlines, and management guidance."], confidence, sourcesUsed };
}
...
function redFlags(input: { historical: FinancialYear[]; scenarios: Scenario[]; filings: NewsRow[]; news: NewsRow[]; wacc: WaccDetail; sources: Record<string, SourceState> }) {
  ...
  if (!latest?.shares && input.sources.FinnhubProfile !== "ok") flags.push("Shares outstanding came from fallback data; verify diluted shares manually.");
...
function costOfDebt(history: FinancialYear[]) { const values = history.slice(0, 3).map((row) => row.interestExpense != null && row.totalDebt ? row.interestExpense / row.totalDebt : null).filter(isNumber).filter((value) => value > 0 && value < 0.25); return average(values); }
function trendGrowth(history: FinancialYear[]) { const latest = history[0]?.revenue; const old = history[Math.min(3, history.length - 1)]?.revenue; if (latest != null && old != null && old > 0 && history.length > 1) return Math.pow(latest / old, 1 / Math.min(3, history.length - 1)) - 1; return average(history.slice(0, 4).map((row) => row.revenueGrowth).filter(isNumber)); }
function sourceFallbackGrowth(sources: Record<string, SourceState>) { sources.DerivedGrowthFallback = "ok"; return 0.03; }
function fallbackFcfMargin(overview: Record<string, string> | null, sources: Record<string, SourceState>) { const profitMargin = number(overview?.ProfitMargin); sources.DerivedFcfMargin = profitMargin != null ? "ok" : "unavailable"; return profitMargin == null ? null : clamp(profitMargin * 0.82, 0.015, 0.34); }
function neutralNetDebtFallback(sources: Record<string, SourceState>) { sources.NeutralNetDebtFallback = "ok"; return 0; }
function unavailable(symbol: string, sources: Record<string, SourceState>, message: string) { return { symbol, companyName: null, price: null, source: null, status: "unavailable", sources, generatedAt: new Date().toISOString(), profile: { sector: null, industry: null, description: null, marketCap: null, analystTargetPrice: null, beta: null, peRatio: null, profitMargin: null, revenueTtm: null, cik: null }, metrics: { revenue: null, grossMargin: null, ebitda: null, ebit: null, taxes: null, operatingCashFlow: null, capex: null, freeCashFlow: null, fcfMargin: null, workingCapital: null, cash: null, totalDebt: null, netDebt: null, shares: null, revenueGrowthTrend: null }, wacc: { riskFreeRate: null, beta: null, equityRiskPremium: EQUITY_RISK_PREMIUM, costOfEquity: null, preTaxCostOfDebt: null, afterTaxCostOfDebt: null, taxRate: null, equityValue: null, debtValue: null, equityWeight: null, debtWeight: null, wacc: null }, historical: [], scenarios: [], forecasts: [], sensitivity: { waccValues: [], terminalGrowthValues: [], rows: [] }, predictions: [], research: { valuationSummary: message, investmentMemo: message, businessSummary: message, newsSummary: message, filingSummary: message, macroSummary: message, catalysts: [], risks: [message], whatToVerify: [], confidence: 0, sourcesUsed: [] }, evidence: { news: [], filings: [] }, sourceLinks: [] }; }
async function fetchJson<T>(url: string, headers?: HeadersInit): Promise<T | null> { const response = await fetch(url, { headers: { Accept: "application/json", ...(headers ?? {}) }, signal: AbortSignal.timeout(12_000), cache: "no-store" }); if (!response.ok) return null; return (await response.json()) as T; }
function newsRow(input: { title: string; source: string; url: string; summary?: string; publishedAt: string }) { const cleanTitle = truncate(input.title || "Untitled", 240); const cleanSummary = truncate(input.summary || "", 700); const textValue = `${cleanTitle} ${cleanSummary}`; return { title: cleanTitle, source: input.source || "News", url: input.url || "", category: category(textValue), severity: severity(textValue), publishedAt: input.publishedAt || new Date().toISOString(), summary: cleanSummary } satisfies NewsRow; }
...
function absOrNull(value: number | null) { return value == null ? null : Math.abs(value); }
